...
export default function V2SessionsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await apiFetch<{ sessions: V2Session[] }>(
        "/api/sessions/v2",
      );
  ...
  const active = sessions.filter(
    (s) => s.status === "OPEN" || s.status === "DRAFT" || s.status === "PAUSED",
  );
  ...
  function statusLabel(status: string) {
    ...
    if (status === "DRAFT")
      return <Badge className="bg-slate-100 text-slate-600">Borrador</Badge>;
    if (status === "PAUSED")
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-500">
            Inventario por posición física.
          </p>
        </div>
        <Link href="/sessions/v2/new">
          <Button size="sm">
            <MapPin size={14} /> Nueva sesión V2
          </Button>
        </Link>
      </div>
            ...
            <Card
              ...
              onClick={() => {
                if (session.status === "OPEN" || session.status === "DRAFT")
                  router.push(`/sessions/v2/${session.id}/scan`);
              ...
              <CardContent className="flex items-center gap-4 py-4">
                ...
                <div className="text-right text-sm">
                  <p className="font-medium">
                    {session._count.sessionPositions}
                  </p>
                ...
                {session.status === "REVIEW" && (
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      router.push(`/sessions/v2/${session.id}/review`);
                    }}
                  >
                    Revisar
                  </Button>
