...
export default function V3SessionsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await apiFetch<{ sessions: V3Session[] }>(
        "/api/sessions/v3",
      );
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold tracking-tight">Sesiones V3</h1>
          <p className="text-sm text-slate-500">
            Inventario por cajas (sin posiciones físicas).
          </p>
        </div>
        <Link href="/sessions/v3/new">
          <Button size="sm">
            <Package size={14} /> Nueva sesión V3
          </Button>
        </Link>
      </div>
