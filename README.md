...
- Vista previa de rack con resumen operativo, distribución frontal SVG, vista lateral de profundidad y detalle desplegable de posiciones.
- Diseñador visual de rack con flujo guiado: configurar compartimentos desde configuración rápida (niveles, columnas, filas y profundidades), revisar en vistas frontal/lateral accesibles, editar propiedades y generar posiciones físicas.
...
- **Sesiones V4 — Inventario por cajas con ingreso rápido:** Flujo en una sola pantalla para conteo rápido. Seleccionar importación → seleccionar cajas con input de rangos (ej: `1-50, 55, 60-70`) → digitar código o nombre de producto (búsqueda case-insensitive) → ver nombre y código proveedor → ingresar cantidad por caja → total automático. Permite marcar cantidades diferentes entre cajas para editar cada caja individualmente y conserva los valores al cambiar el rango. Soporta hasta 900+ cajas con presets rápidos (1-10, 1-50, 1-100, 1-200). Muestra cajas ya contadas en verde y permite agregar manualmente otros productos a una caja ya contada, incluso mientras su ronda está en revisión, siempre que no esté aprobada. Revisión con dos pestañas: resumen por producto y detalle por caja. Reutiliza tablas Import/Pallet/Box existentes.
- **Sesiones V5 — Captura rápida general:** Flujo independiente de importaciones, pallets y ubicaciones. El operador se identifica, ingresa un código de producto, consulta la descripción guardada, registra cantidad de cajas y unidades por caja, obtiene el total automático y puede agregar un comentario opcional. Cada registro queda guardado con operador, fecha y detalle en el historial de la sesión.
